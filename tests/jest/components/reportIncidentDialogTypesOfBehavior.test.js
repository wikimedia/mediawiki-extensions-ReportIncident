...
describe( 'Report Incident Dialog Types of Behavior', () => {
	...
	it( 'showSomethingElseDetailsCharacterCountLeft should be false when something else field hidden', () => {
		const wrapper = renderComponent();
		wrapper.vm.somethingElseDetailsCharacterCountLeft = 1;
		return nextTick( () => {
			expect( wrapper.vm.showSomethingElseDetailsCharacterCountLeft ).toBe( false );
		} );
	} );

	it( 'showSomethingElseDetailsCharacterCountLeft should be false when counter is the empty string', () => {
		const wrapper = renderComponent();
		const store = useFormStore();
		store.inputBehavior = Constants.harassmentTypes.OTHER;
	...
	it( 'showSomethingElseDetailsCharacterCountLeft should be true when counter is a number and the something else field is shown', () => {
		const wrapper = renderComponent();
		const store = useFormStore();
		...
		return nextTick( () => {
			expect( wrapper.vm.showSomethingElseDetailsCharacterCountLeft ).toBe( true );
		} );
	} );
	...
	it( 'showSomethingElseDetailsCharacterCountLeft should be true when counter is a number', () => {
		const wrapper = renderComponent();
		const store = useFormStore();
